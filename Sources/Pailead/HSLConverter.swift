...
public class HSLConverter {
    /// Convert a pixel into hsl
    ///
    /// - Parameter pixel: The pixel to convert
    /// - Returns: HSL values on a [0, 1] scale
    ...
    func pixelFor(hue : Float, saturation : Float, lumience : Float) -> Pixel {
        ...
        let secondComponentWeight = 1.0 - ((hue * 6.0).truncatingRemainder(dividingBy: 2.0) - 1.0).magnitude
